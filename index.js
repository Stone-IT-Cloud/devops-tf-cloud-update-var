...
async function main() {
    try {
        ...
        const terraformListVariablesEndpoint =
            'https://' +
            terraformHost +
            '/api/v2/vars/?filter[organization][name]=' +
            organizationName +
        ...
        const terraformVariableUpdateEndpoint =
            'https://' + terraformHost + '/api/v2/vars/' + variableId;
        const request = {
            data: {
                id: variableId,
                attributes: {
        ...
        axios.patch(terraformVariableUpdateEndpoint, request, options).then(
            (response) => {
